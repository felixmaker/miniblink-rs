...
typedef void(MB_CALL_TYPE* mbCanGoBackForwardCallback)(mbWebView webView, void* param, enum MbAsynRequestState state, BOOL b);
typedef void(MB_CALL_TYPE* mbGetCookieCallback)(mbWebView webView, void* param, enum MbAsynRequestState state, const utf8* cookie);
...
__declspec(selectany) HMODULE g_hMiniblinkMod = NULL;
...
{
    bool needFill = NULL == g_hMiniblinkMod;
    if (!g_hMiniblinkMod) {
        LoadLibraryW(kMbMainDllPath);
        g_hMiniblinkMod = LoadLibraryW(kMbDllPath);
    }
