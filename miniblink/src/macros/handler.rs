...
//                     $($param: $ctype,)*
//                 ) $(-> $creturn)?
//                 where F: FnMut($target, $($type,)*) $(-> $return)? + 'static,
//                 {
//                     let wv: $target = FromFFI::from(wv_ptr);
...
//                     #[allow(unused)]
//                     let r = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| f(wv, $($param,)*)));
//                     $(r.unwrap_or($default))?
//                 }
...
macro_rules! bind_handler {
    ($(
        ...
        $(
            ...
            where
                F: FnMut(RawWebView, $($type,)*) $(-> $return)? + 'static,
            {
                use crate::ffi::*;
                unsafe extern "C" fn shim<F>(
                    wv_ptr: miniblink_sys::wkeWebView,
                ...
                where F: FnMut(RawWebView, $($type,)*) $(-> $return)? + 'static,
                {
                    let wv= FromFFI::from(wv_ptr);
                    let cb: *mut F = c_ptr as _;
                    let f = &mut *cb;
                    $(
                        let $param: $type = FromFFI::from($param);
